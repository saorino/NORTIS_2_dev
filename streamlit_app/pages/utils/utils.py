...
def load_and_prepare_csv_data(file_path):
...
def load_all_csv_in_directory(folder_path):
    folder = Path(folder_path)
    return {file.stem: load_and_prepare_csv_data(file) for file in folder.glob("*.csv")}
...
def load_mobility_data_csv(csv_directory, geojson_directory, geojson_files):
    # Carrega os dados CSV
    mobility_data = load_all_csv_in_directory(csv_directory)
    # Carrega os dados GeoJSON e adiciona ao dicionário
    for geojson_file in geojson_files:
        file_path = os.path.join(geojson_directory, geojson_file)
        if os.path.exists(file_path):
